...
from src.import_test import i_test
from src.anIres import anIres
from src.aom_overlap import AOM_overlap_calculation
from src.aom_overlap import calculate_overlap_S_matrix
...
class single_molecule:
    ...
    def __init__(self,filename,src='file',input_var=None):
        if src=='file':
            ...
            #for index in range(len(self.species)):
                #print(self.species[index], self.x[index], self.y[index], self.z[index])
        elif src=='variable':
    ...
    def resolve_pvecs(self,STO_matrix=None):
        ...
        query = tree.query_ball_point(coordinates, r=3.55)
...
#[40, 43]]
        ...
        for i in range(len(atomlist)):
            ...
            neighbourlist=[[0 for h in range(4)] for p in range(4)]
        ...
        #            #print('length is:', len(query[neighbourlist[0][0]-1]))
        ...
        #                break
               ...
               #added new conditional statement to select nearest neighbours of ketone oxygen's nearest neighbour, 
               #instead of using the ketone oxygen itself to construct the pvecs - FI 01/2024
                #print(query[neighbourlist[0][0]-1])
                for k in sorted(query[neighbourlist[0][0] - 1]):
                    if neighbourlist[0][0]!=k+1 and atomlist[i]!=k:
                        neighbourlist[0][j]=k+1
                        neighbourlist[1][j]=self.x[k]
                        neighbourlist[2][j]=self.y[k]
                        neighbourlist[3][j]=self.z[k]
            ...
            veca_x,veca_y,veca_z=[neighbourlist[num][0]-neighbourlist[num][2] for num in [1,2,3]]
            vecb_x,vecb_y,vecb_z=[neighbourlist[num][1]-neighbourlist[num][2] for num in [1,2,3]]
...
def Sab(dimer_xyz_file,frag1_AOM_file,frag2_AOM_file,frag1_MO,frag2_MO,AOM_dict,phase=None,preview='test.cube'):
    ...
    elif isinstance(dimer_xyz_file,np.ndarray):
        species,x,y,z =dimer_xyz_file.T
        x = x.astype(np.float)
        y = y.astype(np.float)
        z = z.astype(np.float)
    atoms=len(species)
