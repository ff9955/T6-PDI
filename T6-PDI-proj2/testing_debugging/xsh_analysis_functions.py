...
def state_type_populations(diabat_population_array, distance_file_path, coulomb_barrier_file_path, number_diabats):
    '''
    ...
    #diabat_population_array = diabatic_populations(coeff_file_path, number_diabats)
    #calculate time-dependent diabatic populations using the coeff file
    ...
    #-------------------------------------------------------------------------------
    #TEMPORARILY DIVIDING UP THE DIFFERENT TYPES OF EXCITON POPULATIONS

    exciton_populations = diabat_population_array[:,-number_excitons:]

    #interfacial_exciton_pops = np.sum(exciton_populations[:, :3], axis=1)
    #boundary_exciton_pops = np.sum(exciton_populations[:, -3:], axis=1)
    #bulk_exciton_pops = np.sum(exciton_populations[:, 3:-3], axis=1)
    ...
    interfacial_CT_index = np.where(coulomb_array == sorted_coulomb_array[0])[0][0]
    ...
    return CSS_populations, exciton_populations, interfacial_CT_populations, almost_interfacial_CT_populations
