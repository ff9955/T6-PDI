...
def initial_XT_character(sh_file_path, pseudoH_file_path, number_diabats, number_excitons, recomb):
    '''
    ...
    Input: sh_file_path: string, path to run-sh-1.log file; pseudoH_file_path: string, path to run-pseudo-hamilt-1.xyz file; number_diabats: int, number of diabats composing the electronic wavefunction; number_excitons: int, out of the total number of diabats, how many of them are frenkel excitons? recomb: bool, whether the neutral GS has been included in the state space or not, as this affects the indices of the XT diabats in the wavefunction
    ...
    if not recomb:
        ...
        XT_character = np.sum(adiabat_wavefunction[-number_excitons-1:-1]**2)
    #calculate total populations of exciton diabats of the active surface wavefunction only

    return XT_character, adiabat_energy

def trajectory_initial_XT(trajectory_list , sh_file_path, pseudoH_file_path, target_dir, number_diabats, number_excitons, recomb):
    '''
    ...
    for trajectory in trajectory_list:
        ...
        xt_char, energy = initial_XT_character(f'{target_dir}/run-fssh-{trajectory}/{sh_file_path}', f'{target_dir}/run-fssh-{trajectory}/{pseudoH_file_path}', number_diabats, number_excitons, recomb)
...
def diabatic_populations(file_path, number_diabats):
    '''
...
def get_general_output(file_path, number_columns, excess_columns, number_diabats):
    '''
    ...
    adiabat_array = xyz_parser(file_path, number_columns)
    adiabat_array = adiabat_array[:, excess_columns:]
    ...
    output_array = np.zeros((number_timesteps, number_diabats))
    ...
    for counter, index in enumerate(array_indices):
        ...
        current_output = adiabat_array[lower_index:upper_index, 0]
        #getting adiab array section corresponding to one time step

        output_array[counter, :] = current_output
        #assign a row of populations to a certain timestep

    return output_array


def state_type_populations(diabat_population_array, distance_file_path, coulomb_barrier_file_path, eIPR, hIPR, number_diabats):
    '''
    ...
    interfacial_CT_index = np.where(coulomb_array == sorted_coulomb_array[0])[0] #[0] generalising to 2D, multiple iCT diabats
...
#    second_interfacial_index = np.where(coulomb_array == sorted_coulomb_array[1])[0][0]
#    third_interfacial_index = np.where(coulomb_array == sorted_coulomb_array[2])[0][0]

#    almost_interfacial_CT_populations = diabat_population_array[:,second_interfacial_index] + diabat_population_array[:,third_interfacial_index]
...
#    max_coulomb_value = max(coulomb_array)
    ...
    CSS_indices = CSS_indices_by_distance
    ...
    interfacial_CT_populations = np.sum(interfacial_CT_populations, axis=1) #generalising to 2D, multiple iCT diabats
    ...
    #defining cutoff distance based on diameters of electron/hole clouds, beyond which charge separation deemed to have irreversibly occured

    full_sep_indices = np.where(distance_array > cutoff_distance)[0]
    full_sep_populations = diabat_population_array[:, full_sep_indices]
    full_sep_populations = np.sum(full_sep_populations, axis=1)

    return CSS_populations, full_sep_populations, exciton_populations, interfacial_CT_populations
...
def IPR(population_frame, number_diabats, number_excitons, number_donors, number_acceptors):
    '''
    ...
    #only calculating exciton IPR if total population is > 0, otherwise it's meaningless
    if np.sum(exciton_populations) > 0:
    ...
    number_DA_molecules = int(number_donors*number_acceptors)
    ...
    acceptor_site_pops = np.zeros(number_acceptors)
    donor_site_pops = np.zeros(number_donors)

    for j in range(number_donors):
    #i get the electron/hole IPR by defining new electronic states, which correspond to the hole localised on one molecule and the electron being wherever, and vice versa
        ...
        donor_sites = population_frame[j*number_donors : j*number_donors + number_donors]
    ...
    for j in range(number_acceptors):

        acceptor_sites = population_frame[np.arange(j, number_CT_states ,number_acceptors)]
    ...
    #to prevent NaN only renormalise if sum is more than zero, if zero then set IPR to zero as it's meaningless anyway
    if (np.sum(acceptor_site_pops) > 0) and (np.sum(donor_site_pops) > 0):
...
def single_trajectory_IPR(coeff_file_path, number_diabats, number_excitons, number_donors, number_acceptors):
    '''
    ...
    for frame, population in enumerate(diabat_populations):
        ...
        exciton_IPR, acceptor_IPR, donor_IPR = IPR(population, number_diabats, number_excitons, number_donors, number_acceptors)
...
def diabat_locations(donor_coords, acceptor_coords, exciton_coords, population_frame, number_diabats, number_excitons, number_donors, number_acceptors):
    '''
    ...
    if (np.sum(donor_site_pops) > 0) and (np.sum(acceptor_site_pops) > 0):
        ...
        average_donor_location, average_acceptor_location = 0, 0
...
def single_trajectory_diabat_locations(donor_coords, acceptor_coords, exciton_coords, coeff_file_path, number_diabats, number_excitons, number_donors, number_acceptors):
    '''
    ...
    for frame, population in enumerate(diabat_populations):
        #iterating over every timestep for which the diabatic populations were printed
            ...
            xt_loc, acc_loc, don_loc = diabat_locations(donor_coords, acceptor_coords, exciton_coords, population, number_diabats, number_excitons, number_donors, number_acceptors)
