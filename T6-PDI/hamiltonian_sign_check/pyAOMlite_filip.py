...
config={
    'label': 'PDI',
    'single_mol_xyz': 'solar_cell_pic/excitons/exciton_crystal_317.xyz',
    'basis': 'DZVP-GTH',
    'potential': 'GTH-PBE',
    'MO': 71,
...
dimer_dir='/scratch/fivanovic/X-SH/T6-PDI/defect_prep/defect_nve_runs/pdi_geoms_nve_1ns/'

# define the AOM files for both dimer fragments
frag1_AOM_file='output/PDI/AOM_LUMO_COEFF.dat'
frag2_AOM_file='output/PDI/AOM_LUMO_COEFF.dat'
# define MOs
frag1_MO=71
frag2_MO=71
#--------------------------------------------------------------------------------
name = [f'PDI_dimer-{q}.txt' for q in range(100,1100)]
#name = ['pdi_pair.xyz', 'pdi_pair_defect1.xyz', 'pdi_pair_defect2.xyz']
...
for file in name:
    ...
    val=Sab(dimer_xyz_file,frag1_AOM_file,frag2_AOM_file,frag1_MO,frag2_MO,AOM_dict,phase=None)
    #val=Sab(dimer_xyz_file,frag1_AOM_file,frag2_AOM_file,frag1_MO,frag2_MO,AOM_dict,phase=(1,1),preview=f'{file[:-4]}.cube')
    Sab_array.append(val)
    print(f'{file} Sab: {val*9064}')

print(np.mean(abs(np.array(Sab_array))*9064))
