...
echo $(($atoms_per_acceptor + $atoms_per_acceptor)) >> PDI_dimer-$current_timestep.txt
echo " " >> PDI_dimer-$current_timestep.txt

sed -n "$line_counter, $((line_counter + $atoms_per_acceptor - 1))p" run-pos-1ps.xyz  >> PDI_dimer-$current_timestep.txt
