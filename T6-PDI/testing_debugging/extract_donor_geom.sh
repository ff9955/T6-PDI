...
current_timestep=$((current_timestep*$timesteps_skipped*100))

echo $(($atoms_per_donor + $atoms_per_donor)) >> T6-dimer-$current_timestep.txt
echo " " >> T6-dimer-$current_timestep.txt

sed -n "$line_counter, $((line_counter + $atoms_per_donor - 1))p" run-pos-1.xyz  >> T6-dimer-$current_timestep.txt
